 ...
 * Horizontal scrollable category badges — matches Odoo category_badges_horizontal.
      ...
      <div className="px-4">
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {[
            ...
            { icon: "fa-mobile", label: "Smartphones" },
            { icon: "fa-tv", label: "Monitors" },
            { icon: "fa-bolt", label: "Flash Deals" },
            { icon: "fa-clock-o", label: "Smart Watches" },
          ].map((item) => (
            <Link
              key={item.label}
              href={`/shop?q=${item.label.toLowerCase()}`}
              className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold no-underline whitespace-nowrap transition-all hover:-translate-y-0.5"
              style={{
                background: "#f8f9fa",
                color: "#374151",
            ...
            >
              <i className={`fa ${item.icon} text-sm text-blue-600`} />
              {item.label}
            </Link>
          ))}
        </div>
